...
当前实现面向学习、验证和架构拆解：代码量小，模块边界清楚，测试覆盖核心协议和安全边界。它已经可以完成基础文件读写、精确替换、命令执行、写前确认、编辑后的测试验证、多文件检索、上下文压缩和 TodoWrite 式任务规划；还没有实现可观测 hooks、trace 数据回流或持续 eval 平台。
...
- 提供 eval runner、5 个 P2 基准任务和 5 个 P3 多文件任务；真实基线需要配置 `ARK_API_KEY` 和 `ARK_MODEL` 后运行。
...
当前已实现 eval 任务格式、5 个 P2 基准任务、5 个 P3 多文件任务和 runner：
...
runner 会先构建项目，再用真实 Agent Loop 执行任务，并把结果写入 `evals/results/{timestamp}.json`。真实 eval 需要有效的 `ARK_API_KEY` 和 `ARK_MODEL`；没有密钥时只能运行 schema 和 runner 单测，不能生成正式 baseline。
...
真实 LLM eval 会受模型输出波动影响；这里记录的是一次实测快照，不代表持续趋势。持续 eval、repeat 和退化门禁会在后续 P4 阶段实现。
