...
一个用 TypeScript 从零实现的极简 Coding Agent。项目目标不是复刻完整 IDE Agent，而是把最核心的 agentic loop 跑通：CLI/TUI 接收用户任务，调用 OpenAI-compatible `chat/completions`，解析模型返回的 `tool_calls`，通过 Harness 统一处理权限、安全规则、工具执行和编辑后验证，再把工具结果回传给模型继续决策。
...
TUI 支持单行输入、运行状态、消息流、工具摘要、TODO 展示和内联 `y/n` 权限确认。退出 TUI：输入 `.exit` 或按 Ctrl+C。当前 TUI 不是完整 IDE 界面，也不提供流式输出、GUI diff 或插件市场。
...
```mermaid
flowchart TD
    User[User / CLI + Ink TUI] --> CLI[src/index.ts]
...
| P5 final | `configured-model` | 10 | 9/10 | 5.3 | — | `evals/results/2026-06-14-final.json` |

P5 发布前最终结果复用此前的全量 run（未重跑以避免引入新的随机波动），完整复盘见 `docs/retrospective.md`。
...
- 默认文件、检索、规划和命令工具
- CLI 单次任务、基础 Ink TUI 和本地 npm bin 入口
- 权限确认、基础命令拦截、工作目录边界
- 编辑后测试验证和失败重试状态
- Harness 统一执行控制层
...
- 技术文章（`docs/article-draft.md`）和项目复盘（`docs/retrospective.md`）
