...
当前实现面向学习、验证和架构拆解：代码量小，模块边界清楚，测试覆盖核心协议和安全边界。它已经可以完成基础文件读写、精确替换、命令执行、写前确认、编辑后的测试验证、多文件检索、上下文压缩、TodoWrite 式任务规划、可观测 hooks/trace，以及基于 trace 的持续 eval 报告和退化门禁。
...
- 长文件读取会默认截断，并支持 `offset` / `limit` 分段读取。
- 长对话会在超过阈值时压缩早期上下文，保留最近消息继续执行。
- `todo_write` 可让模型维护结构化 TODO，CLI 会在有计划时展示当前进度。
- 提供 eval runner、5 个 P2 基准任务、5 个 P3 多文件任务、suite/repeat/mock eval、trace 汇总、报告和 baseline check；真实 eval 需要配置 `ARK_API_KEY` 和 `ARK_MODEL` 后运行。
...
| Run | Model | Passed | Notes |
| --- | --- | --- | --- |
| `2026-06-14T07-17-18-855Z` | `deepseek-v4-flash-260425` | 9/10 | 保存于 `evals/results/2026-06-14-p3.json`；`09-add-tests-for-module` 失败于文件内容期望，生成测试通过但缺少期望文本 `multiple-spaces`。 |
...
| `03-fix-logic-bug` | pass | 5 | 0 |
| `04-add-function` | pass | 3 | 0 |
| `05-refactor` | pass | 4 | 0 |
| `06-grep-fix` | pass | 4 | 0 |
| `07-add-patterned-function` | pass | 5 | 1 |
| `08-cross-file-rename` | pass | 6 | 2 |
| `09-add-tests-for-module` | fail | 8 | 1 |
| `10-implement-from-spec` | pass | 6 | 0 |

真实 LLM eval 会受模型输出波动影响；这里记录的是一次实测快照，不代表持续趋势。P4 持续 eval 支持 `--repeat` 识别 flaky task，并通过 `--baseline ... --check` 在 pass rate、平均 turns、平均 tool calls、flaky rate 和 feedback health 退化时失败。更新正式 baseline 时，应先运行真实 eval，再显式保存 `evals/baselines/p4-continuous.json`。
