...
> 导读：如果你只想理解智能体的主体运行机制，可以先略读本章的协议细节、看小结即可。本章的核心只有一句话——连接外部世界的杂事该集中交给一个「服务层」来管。
...
- **区分错误**：外部服务失败时，要分清「能重试的」和「重试也没用的」，分别处理，而不是一律当成失败。
...
> 想深入到实现细节，见姊妹篇《Claude Code 内核解剖》第 5 章。
