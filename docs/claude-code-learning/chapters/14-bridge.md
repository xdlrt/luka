...
> 导读：如果你只想理解智能体的主体运行机制，可以先略读本章的桥接细节、看小结即可。本章的核心只有一句话——把「运行智能体的内核」和「在哪运行的外壳」拆开，安全关卡永远留在内核里。
...
把智能体从「本地终端」扩展到「编辑器/远程」，会让前面几章的一些机制面临新问题。这里不必深究每种问题的处理细节，只需理解它们为什么会冒出来：

**权限确认在哪发生？** 第 4 章里，确认是在终端弹 y/n。但现在用户可能在编辑器里、在远程客户端那头，那个提示要能跨越距离送达、再把答复送回，还要考虑用户迟迟不回应的情况。

**远程的输入可信吗？** 当任务和文件在远端、指令从本地来，远程客户端传过来的文件路径还可信吗？第 4 章的路径边界检查，在远程场景下更不能松懈，因为输入来自「别处」。

**密钥和工作目录归谁？** 远程执行意味着要重新想清楚：访问凭证存在哪一端、哪一端真正拥有工作目录——这些单机时不言自明的事，跨机器后都要重新审视。

**断线了怎么办？** 网络会断，连接断的瞬间，正在执行的工具该停还是该继续、会话怎么在重连后恢复，这些都是本地进程里根本不会遇到的可靠性问题。
...
> 想深入到实现细节，见姊妹篇《Claude Code 内核解剖》第 8 章。
