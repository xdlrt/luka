...
第 3 章我们已经碰过一次「记忆」这个话题，这里要把界线划得更清楚，因为它关系到智能体会不会「自欺欺人」。

很多人会把智能体的各种临时状态，误当成它的「记忆」或「知识」。比如：

- 「待办清单」——这只是当前任务的规划草稿，不是它记住的事实。
- 「对话摘要」——这是压缩历史的产物（第 3 章），可能失真。
- 「运行记录」——这是用于复盘的日志，不是知识库。

这些都是**临时状态**，**绝不能被当成「长期记忆」来对待。** 真正的长期记忆，是那种能跨越多次任务、持久保存、下次接着用的知识——而一个核心实现，通常**根本没有**这种能力，它也不该假装自己有。

为什么这条界线这么重要？因为一旦模糊，智能体就可能开始「编故事」：把一份可能失真的摘要，当成铁打的事实去汇报；把一张临时的待办清单，当成「已经完成的工作」去声称。这正是第 3 章那条原则的延续——**真实发生过的事（铁证）的地位，永远高于任何对信息的加工（二手转述）。**

成熟产品确实在探索真正的长期记忆——把用户偏好、项目知识持久存下来。但这是一套复杂的系统，牵扯到这些信息存哪、怎么更新、怎么删除、可信度如何、升级时旧数据怎么迁移。在没把这些问题想透之前，硬给智能体安一个「长期记忆」的名头，只会让它变得不可靠。**诚实地承认「我只记得这次任务」，好过虚假地宣称「我什么都记得」。**

## 密钥:一条老红线的再次现身

配置里几乎一定包含敏感信息——尤其是访问模型用的密钥。这条红线，第 6 章（外部服务）、第 10 章（输出）都讲过，这里第三次出现，因为它确实贯穿全书：

**密钥这类敏感信息，绝不能写进文档、不能进入发给模型的内容、不能出现在运行记录里。**

它只能安静地待在配置/密钥层，由需要它的地方在内部取用。下一章讲运行记录时，我们会看到为此专门设计的「脱敏」机制——在任何信息被记录下来之前，自动把疑似密钥的部分抹掉。
