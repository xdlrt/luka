...
**独立阅读建议**：本章可独立阅读，记住「模型每轮看到的是被重新组织的上下文」即可。
**联读建议**：与第 1 章（`prompt_too_long` 与 `reactive_compact_retry` 的来源）连读，压缩与恢复路径互为表里。
...
- **autocompact（自动压缩）**：历史整体接近阈值时，把早期对话全量摘要重写。
- **microcompact（微压缩）**：只把又大又旧的工具结果清成占位符，不重写整段历史。
- **reactive compact（响应式压缩）**：请求被服务端以「上下文过长」拒绝后被动触发的救场压缩。
- **预警梯度 / buffer**：从上下文窗口层层减去的安全余量，给「压缩动作本身」留施工空间。
- **预取（prefetch）**：把相关记忆检索藏进流式输出与工具执行的等待时间里，不阻塞主路径。
...
> 证据标签：本章 `// src/...` 路径标注的代码块是 **[本仓库真实代码]**；讲 Claude Code 快照阈值与流程的常量、逻辑是 **[快照推断]**；归纳出来的类型签名是 **[阐释性重构]**。下面这段属于 **[快照推断]**。
...
科普书说压缩时要叮嘱模型「别编造完成的工作」。生产级的摘要 prompt 远不止一句叮嘱——它规定了一套**九段式结构**，强制模型把可继续工作所需的一切都覆盖到（推断，段落标题为快照中可见的命名）：
...
一个最小例子能直观看出这套压缩「保留了什么、丢弃了什么」：

```text
压缩前历史（约 40 轮）：
  user: 帮我把 parser 的报错信息改成中文
  assistant: [读 parser.ts] [grep "throw new Error"] [edit×6] ...
  tool: <6 个文件的完整 diff、3 次失败的测试输出、上千行 grep 结果>
  ... 几十轮工具往返 ...

      ↓ 九段式摘要（只保留可继续工作所需）

assistant(Context summary):
  Primary Request: 把 parser 报错信息中文化
  Files and Code: parser.ts 第 12/45/88 行的 throw 已改；lexer.ts 待改
  Errors and fixes: 第 45 行漏了模板字符串转义，已修
  Current Work: 正在改 lexer.ts 的 3 处报错
  Optional Next Step: 改完 lexer.ts 后跑 npm test 验证

      ↓ 保留尾部消息（最近 N 条原样保留）

user: 等下，第 88 行那个先别动
assistant: 好的，我回退第 88 行……
```

中间几十轮的原始工具结果被摘要替换，但「改到哪、下一步、用户最新意图」一条不丢——这正是「有损交接、不丢连续性」。
