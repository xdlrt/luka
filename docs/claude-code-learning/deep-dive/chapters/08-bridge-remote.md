...
**独立阅读建议**：本章可独立阅读，记住「内核执行工具与裁决权限不能搬家，外壳只是壳」即可。
**联读建议**：与第 4 章（远程权限仍走本地裁决）、第 5 章（elicitation 同源冒泡）、第 7 章（`remote_agent`）、第 9 章（远程成本观测）连读。

## 前情提要

科普书第 14 章给了一个干净的画面：把「运行智能体的内核」和「在哪运行、被谁驱动的外壳」拆开。终端、IDE、远程客户端、SDK 都只是外壳；真正执行工具、做权限裁决的内核不能搬家。

那一章停在结构示意。本章要钻进快照里的 bridge（约 30 个文件）、remote 与 server 模块，看这层「会话协议」到底由哪些消息组成、JWT 携带什么、断线后队列怎么处理、远程权限请求怎么在内核里落地、SDK 消费者拿到的是什么结构。你会发现：桥接层的复杂度几乎全部来自「网络不可靠 + 安全裁决不能外包」这两条约束的乘积。

> 路径约定（重要）：本章凡指代 Claude Code 快照的文件，一律写成不带 `src/` 前缀的相对路径（如 `bridge/bridgeMessaging.ts`），它们属 **[快照推断]**，**本仓库并不存在**这些文件。只有带 `// src/...` 标注的代码块才是本仓库可运行的真实代码。两者来源不同，请勿混淆。
...
- **control plane（控制平面）**：协调会话生命周期的带外指令（`control_request`/`control_response`），权限请求走这里。
- **content stream（内容流）**：会话里真实发生的消息（user/assistant/result/system/tool_progress）。
- **合成上下文（synthetic context）**：远程工具调用本地没有真实 assistant message，内核显式造一个再走本地裁决。
- **`viewerOnly`**：纯观看客户端的能力分级——能读事件，但不能 interrupt、不改标题、断线超时被禁用。
- **`detached`**：客户端断了但会话还活着的显式状态，回答「断线时工具继续还是中止」。
...
快照里用一组类型守卫把两者区分开（`bridge/bridgeMessaging.ts`、`remote/SessionsWebSocket.ts`）：

> 证据标签：本章 `// src/...` 路径标注的代码块是 **[本仓库真实代码]**；带 `bridge/`、`remote/`、`server/` 等**无 `src/` 前缀**路径、讲 Claude Code 快照协议的归纳结构是 **[快照推断]**（本仓库无这些文件）；归纳出来的类型签名是 **[阐释性重构]**。下面这段属于 **[快照推断]**。
...
控制平面的 `subtype` 是快照中可见的协议字面量（`entrypoints/sdk/controlSchemas.ts`，Zod schema）：`initialize`、`set_model`、`set_max_thinking_tokens`、`set_permission_mode`、`interrupt`、`can_use_tool`……内核只认识这些抽象指令；外壳负责把它们渲染成终端行、IDE 面板、SDK 回调或远程 UI。
...
远程模式下有个尴尬现实：工具其实是在远端容器（CCR）里跑的，本地内核**没有**触发这次工具调用的真实 assistant message。但本地权限链路（`ToolUseConfirm`）又要求一个 `AssistantMessage`。快照的解法是显式**合成**一个（`remote/remotePermissionBridge.ts`）：
...
`can_use_tool` 请求和它的回复，字段都来自快照中可见的命名（`controlSchemas.ts` + `bridge/bridgePermissionCallbacks.ts`）：

```typescript
// 阐释性重构——权限请求/回复的字段为快照中可见的命名，非逐字源码
...
最外层是 `WorkSecret`——服务端下发的、base64url 编码的一段 JSON，描述「这次工作要在什么环境、用什么 token、连哪个 ingress」（`bridge/types.ts` + `workSecret.ts`）：

```typescript
// 阐释性重构——WorkSecret 的形态基于快照归纳，非逐字源码
type WorkSecret = {
...
token 刷新被做成一台带「代际计数器」的小调度器（`createTokenRefreshScheduler`，常量为快照中可见的命名）：
...
**Flush gate**（`bridge/flushGate.ts`）解决的是「初始历史回放」与「新消息」抢道的问题。会话刚起时，历史消息要用**一次** HTTP POST 整体刷给服务端；这段时间里新产生的消息必须排队，否则会和历史消息交错到达：
...
**重连预算**在 `SessionsWebSocket` 里是一组快照中可见的常量与分类：
...
把这些拼起来，远程会话其实在跑一台状态机。快照的 `server/types.ts` 给出了直连模式的状态枚举：

```typescript
// 阐释性重构——直连会话状态机，状态名为快照中可见的命名
...
SDK 消费者要的是稳定结构，不是 TUI 的彩色文本。快照的 `remote/sdkMessageAdapter.ts` 里 `convertSDKMessage` 把线上的 `SDKMessage` 判别联合翻译成内部渲染类型，返回一个三态结果：
