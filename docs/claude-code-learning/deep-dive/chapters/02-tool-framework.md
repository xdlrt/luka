...
**独立阅读建议**：本章可独立阅读，把工具理解成「给模型的菜单 + 真正干活的实现」两份描述即可。
**联读建议**：与第 5 章（MCP 工具归一化成同一接口）、第 6 章（插件出口治理）连读，能看清工具来源信任链。
...
- **菜单 / 实现分离**：模型只看到工具的 `name`/`description`/`parameters`（菜单），看不到 `execute`/`category`（实现）。
- **投影（projection）**：从完整工具对象里只摘出模型该看的字段、丢弃其余，是单向、唯一的闸门。
- **行为分类**：`isReadOnly` / `isConcurrencySafe` / `isDestructive` 三个正交维度，驱动权限与并发决策。
- **`searchHint` / 工具搜索**：工具过多时不全塞进菜单，模型按关键词搜索才「调出」工具。
- **`ToolUseContext`**：统一注入给工具的运行时环境（中断信号、Agent 身份、权限上下文），工具不得自取。

## 机制深挖：一个工具接口的全貌

科普书把工具实现简化成「名字 + 说明 + 参数 + 执行逻辑 + 权限分类」五样。真实的工具接口要丰富得多。下面是它的改写形态（推断，保留快照中可见的字段命名以体现职责密度）：

> 证据标签：本章 `// src/...` 路径标注的代码块是 **[本仓库真实代码]**；归纳出来的接口类型签名是 **[阐释性重构]**。下面这段属于 **[阐释性重构]**。
...
- UI 渲染：最小实现完全没有，界面层（CLI / TUI）单独处理。
