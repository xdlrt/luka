...
**独立阅读建议**：本章可独立阅读，记住 Skill 是「按需取用的本领包」、插件是「打包好的扩展包」即可。
**联读建议**：与第 2 章（工具搜索同源的按需加载）、第 4 章（校验顺序的安全姿态）、第 7 章（agents 出口的 allowedTools）连读。
...
- **frontmatter**：`SKILL.md` 头部的 YAML 元数据，是声明能力、权限作用域、触发控制的契约。
- **`allowedTools`**：Skill/子 Agent 运行时能用哪些工具的权限作用域，不是提示词建议。
- **按需加载**：加载期只读 frontmatter 算 token，完整正文延迟到 Skill 真正被调用时才读。
- **出口（component）**：插件可接入的 commands/agents/skills/hooks/output-styles 等类别，各自分路径治理。
- **路径穿越（path traversal）**：清单里用 `../` 指向包外路径的攻击，校验顺序里必须最先查。
- **供应链治理**：`sha` 版本锁定、blocklist、policy、trust——管「外部代码来源可信、版本可钉」。

## Skill：frontmatter 即契约

科普书说 Skill 是「一组资源」。它的入口是一个 `SKILL.md`，文件头部的 frontmatter（YAML 元数据）才是真正的契约。生产级解析出的字段远超「名字 + 说明」（推断，字段为快照中可见的命名）：

> 证据标签：本章 `// src/...` 路径标注的代码块是 **[本仓库真实代码]**；讲 Claude Code 快照 Skill frontmatter、插件清单、校验顺序的结构与逻辑是 **[快照推断]**；归纳出来的类型签名是 **[阐释性重构]**。下面这段属于 **[快照推断]**。
...
科普书说插件能接入「命令/agent/skill/hook/输出风格」多个出口。深度层最值得看的是：这些出口在数据结构上是**分开记录**的，而不是塞进一个统一目录（推断，结构为基于快照归纳的形态）：
