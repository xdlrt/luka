...
科普书说"网络抖动可以重试"。生产级的重试是一套精心标定的退避数学。先看延迟怎么算（推断，常量与公式为真实值）：
...
源码里维护了一个"前台来源"集合——只有用户**正在阻塞等待结果**的请求才在容量过载时重试（推断，来源标识为真实值）：
...
科普书说 MCP 是"通用插座"。它的"通用"，体现在一个连接管理器要吸收**五种**底层传输的差异（推断，传输类型为真实值）：
...
科普书说 OAuth"安全地拿 token"。深度层要看的是：**认证失败不是一种错误，而是好几种，每种的恢复路径不同。** 服务层把外部错误分成一个判别联合（推断，结构为真实形态）：
