...
> 一本写给对 AI 编程工具机制感兴趣的技术读者的书：当你对一个 AI 说「帮我修个 bug」，屏幕背后到底发生了什么。
...
本书面向对 AI 编程工具机制感兴趣的技术读者与工程师。你不需要读过任何源码，也不需要某门语言的实战经验——我们用故事和原理讲清楚每一个设计，把工程细节留到需要时再展开。
...
本书对 Claude Code 内部设计的描述，部分基于对其公开行为与架构形态的研究与推断。凡属推断之处，正文会以「（基于公开行为推断）」标注。本书目标是帮助读者理解一个编程智能体背后的设计思想与权衡，而非提供其官方实现文档。

## 姊妹篇

如果你已经读完本书，想继续钻研控制流、数据结构、协议细节和安全边界，可以接着读深度补充册：

- [《Claude Code 内核解剖》](/deep-dive/)：面向资深工程师的硬核技术篇，使用改写伪代码、结构化数据类型和本项目最小实现代码片段，深入分析查询状态机、工具框架、Bash 安全、MCP、插件、多 Agent、桥接和可观测系统。

想深挖某个主题时，可以直接跳到深度册对应章节：

- 想深挖**循环 / 状态机**（本书第 1 章）→ 深度册第 1 章「查询循环的真实状态机」
- 想深挖**工具协议**（本书第 2 章）→ 深度册第 2 章「工具执行框架全貌」
- 想深挖**上下文与记忆 / 状态治理**（本书第 3、12 章）→ 深度册第 3 章「上下文工程与记忆系统」
- 想深挖**权限与安全**（本书第 4 章）→ 深度册第 4 章「Bash 安全与权限分层」
- 想深挖**外部连接 MCP / LSP / OAuth**（本书第 6 章）→ 深度册第 5 章「外部协议」
- 想深挖**Skill 与插件治理**（本书第 5、7 章）→ 深度册第 6 章「扩展治理」
- 想深挖**子智能体与任务编排**（本书第 8 章）→ 深度册第 7 章「多 Agent 编排」
- 想深挖**桥接 / 远程 / 服务端**（本书第 14 章）→ 深度册第 8 章「桥接、远程与服务端协议」
- 想深挖**可观测与评估**（本书第 13 章）→ 深度册第 9 章「可观测、成本与遥测」

更完整的双向章节对照，见深度册附录《与〈拆解 Claude Code〉章节对照》。
